...
MEDIUM_PRIORITY_KEYWORDS = ["multi", "multie fam", ]
...
class SaleEvent:
    ...
    # Tracks how the map location/address was obtained.
    # Examples: listing_address, gps_coordinates, intersection_lookup, photo_ocr_address
    location_method: str = ""
    location_symbol: str = "❓"
    location_raw_data: Dict[str, Any] = field(default_factory=dict)
...
LOCATION_METHOD_LABELS = {
    "listing_address": "Listing Address",
    "gps_coordinates": "Listing GPS",
    "gps_reverse_geocode": "Craigslist GPS",
    "intersection_lookup": "Intersection",
    "photo_ocr_address": "Photo OCR",
    "google_geocode": "Google Geocode",
    "google_cache": "Google Cache",
    "not_found": "Location Not Found",
    "unknown": "Unknown",
}

LOCATION_METHOD_SYMBOLS = {
    "listing_address": "🏠",
    "gps_coordinates": "📍",
    "gps_reverse_geocode": "📍",
    "intersection_lookup": "➕",
...
def set_location_tracking(event: SaleEvent, method: str, raw_data: Optional[Dict[str, Any]] = None) -> None:
    method = method or "unknown"
    event.location_method = method
    event.location_symbol = LOCATION_METHOD_SYMBOLS.get(method, LOCATION_METHOD_SYMBOLS["unknown"])

    if raw_data:
        current = dict(event.location_raw_data or {})
        current.update(raw_data)
        event.location_raw_data = current


def location_method_label(event: SaleEvent) -> str:
    return LOCATION_METHOD_LABELS.get(event.location_method or "unknown", event.location_method or "Unknown location source")


def location_debug_payload(event: SaleEvent) -> Dict[str, Any]:
    payload = {
        "location_method": event.location_method or "unknown",
        "location_method_label": location_method_label(event),
        "location_symbol": event.location_symbol or "❓",
        "source": event.source,
    ...
    if event.location_raw_data:
        payload["raw_location_data"] = event.location_raw_data
    return payload



def pretty_location_details_html(event: SaleEvent) -> str:
    """Return a compact, readable location-details card for popup <details>."""
    payload = location_debug_payload(event)
    raw_data = payload.get("raw_location_data") or {}
    ...
    def fmt_value(value: Any) -> str:
        ...
        if isinstance(value, (list, tuple)):
            return "<br>".join(html.escape(str(v)) for v in value if not is_empty(v))
        if isinstance(value, dict):
            ...
            for k, v in value.items():
                ...
                rows.append(
                    ...
                    f"<span style='color:#666;'>{html.escape(str(k).replace('_', ' ').title())}:</span> "
                    f"<span>{fmt_value(v)}</span>"
                    f"</div>"
                )
            return "".join(rows)
        return html.escape(str(value))

    def row(icon: str, label: str, value: Any, link: Optional[str] = None) -> str:
        if is_empty(value):
            return ""
        if link:
            value_html = f"<a href='{html.escape(link)}' target='_blank' style='color:#1a73e8; font-weight:bold;'>{fmt_value(value)}</a>"
        else:
            value_html = fmt_value(value)
        return (
            "<div style='padding:4px 0; border-bottom:1px solid #ddd;'>"
            f"<div style='font-weight:bold; color:#222;'>{icon} {html.escape(label)}</div>"
            f"<div style='margin-top:2px; color:#333; word-break:break-word;'>{value_html}</div>"
            "</div>"
        )

    method_label = payload.get("location_method_label") or "Unknown"
    symbol = payload.get("location_symbol") or "❓"
    coords = ""
    if not is_empty(payload.get("latitude")) and not is_empty(payload.get("longitude")):
        coords = f"{float(payload['latitude']):.6f}, {float(payload['longitude']):.6f}"

    source_link_label = "View Original Listing" if str(payload.get("source_link", "")).startswith("http") else payload.get("source_link")
    ...
    html_parts = [
        ...
        "<div style='font-weight:bold; font-size:11px; letter-spacing:.5px; margin-bottom:5px; position:sticky; top:0; background:#f7f7f7; padding-bottom:3px;'>LOCATION DETAILS</div>",
        row(symbol, "Found By", method_label),
        row("📍", "Address From Listing", payload.get("raw_address")),
        row("🌎", "Google / Map Address", payload.get("formatted_address")),
        row("📌", "Coordinates", coords),
        row("🆔", "Google Place ID", payload.get("google_place_id")),
        row("🏷", "Source", payload.get("source")),
        row("📄", "Source File", payload.get("source_file")),
        row("🔗", "Source Link", source_link_label, payload.get("source_link") if payload.get("source_link") else None),
        row("🗺", "Google Maps", "Open Map Search", payload.get("google_maps_link") if payload.get("google_maps_link") else None),
    ]

    if raw_data:
        html_parts.append(
            "<div style='padding:4px 0;'>"
            "<div style='font-weight:bold; color:#222;'>🧾 Extracted / Original Data</div>"
            f"<div style='margin-top:2px; color:#333; word-break:break-word;'>{fmt_value(raw_data)}</div>"
            "</div>"
        )
...
def image_to_data_url(path: str) -> Tuple[Optional[str], Optional[str], Optional[int]]:
    """Return a browser-viewable data URL for a local image file."""
    try:
        file_path = Path(path)
        if not file_path.exists() or not file_path.suffix.lower().endswith(IMAGE_EXTENSIONS):
            return None, None, None

        encoded, mime = image_to_base64(str(file_path))
        if not encoded or not mime:
            return None, None, None

        return f"data:{mime};base64,{encoded}", mime, file_path.stat().st_size
    except Exception as exc:
        log(f"Could not create image data URL for {path}: {exc}")
        return None, None, None


def build_image_reader_html(title: str, image_src: str, source_file: str = "") -> str:
    """Small self-contained HTML image reader saved inside JSON for OCR images."""
    safe_title = html.escape(title or "OCR Sale Image")
    safe_file = html.escape(source_file or "")
    ...
    return (
        ...
        "header{position:sticky;top:0;background:#1f1f1f;padding:10px 12px;border-bottom:1px solid #444;}"
        "h1{font-size:15px;margin:0 0 3px 0;}"
        ".file{font-size:12px;color:#bbb;}"
        ".wrap{padding:10px;}"
        "img{display:block;max-width:100%;height:auto;margin:0 auto;border-radius:6px;background:white;}"
        "</style></head><body>"
        f"<header><h1>{safe_title}</h1><div class='file'>{safe_file}</div></header>"
        f"<div class='wrap'><img src='{safe_src}' alt='OCR sale image'></div>"
        "</body></html>"
    )


def image_attachment_payload(event: SaleEvent) -> Optional[Dict[str, Any]]:
    """Embed OCR source images into JSON so they can be viewed without the original file path."""
    if event.source != "Cloud OCR" or not event.source_link:
        return None

    image_src, mime, size_bytes = image_to_data_url(event.source_link)
    if not image_src or not mime:
        return None

    source_file = event.source_file or Path(event.source_link).name
    return {
        "type": "embedded_ocr_image",
        "file_name": source_file,
        "mime_type": mime,
        "size_bytes": size_bytes,
        "data_url": image_src,
        "reader_html": build_image_reader_html(event.title, image_src, source_file),
        "reader_note": "Save reader_html as an .html file to view the embedded image, or use inline_html inside any HTML page.",
        "inline_html": (
            f'<div class="ocr-image-reader">'
            f'<img src="{html.escape(image_src, quote=True)}" alt="OCR sale image" '
...
def geocode_events(events: List[SaleEvent], geocoder: GoogleGeocoder) -> List[SaleEvent]:
    ...
    for event in events:
        ...
        if result:
            ...
            if force_intersection_lookup:
                set_location_tracking(event, "intersection_lookup", {
                    "original_raw_address": raw_query,
                    "google_query": query,
                    "google_result": result,
                })
            elif not event.location_method:
                set_location_tracking(event, "google_geocode", {
                    "google_query": query,
                    "google_result": result,
                })
            else:
                set_location_tracking(event, event.location_method, {
                    "google_query": query,
                    "google_result": result,
                })
            ...
            set_location_tracking(event, "not_found", {
...
def scrape_gsalr(url: str) -> Optional[SaleEvent]:
    ...
    set_location_tracking(event, "listing_address", {
        "streetAddress": street,
        "addressLocality": city,
        "addressRegion": state,
        "postalCode": zipcode,
        "combined_raw_address": address,
    })
    return event
...
def scrape_craigslist(url: str, geocoder: GoogleGeocoder) -> Optional[SaleEvent]:
    ...
    raw_location_data = {
        "meta_geo_city": city,
        "meta_geo_state": state,
        "meta_geo_latitude": lat,
        "meta_geo_longitude": lng,
        "mapaddress": mapaddress,
        "reverse_geocode_result": reverse_geocode_result,
        "formatted_from_coordinates": formatted_from_coords,
    }

    if mapaddress:
        set_location_tracking(event, "listing_address", raw_location_data)
    elif formatted_from_coords:
        set_location_tracking(event, "gps_reverse_geocode", raw_location_data)
    elif lat is not None and lng is not None:
        set_location_tracking(event, "gps_coordinates", raw_location_data)
    else:
        set_location_tracking(event, "unknown", raw_location_data)
...
def scrape_pennlive(url: str) -> Optional[SaleEvent]:
    ...
    set_location_tracking(event, "listing_address", {
        "street_regex_match": street,
        "city_regex_match": city,
        "combined_raw_address": address,
    })
    return event
...
def parse_photo(image_path: Path) -> SaleEvent:
    ...
    extracted_address = extract_photo_address(full_text) or ""
    event = SaleEvent(
        title=extract_photo_title(lines) or "Photo Yard Sale",
        description=raw_text,
        raw_address=extracted_address,
        date_text=date_text or "No Dates Found",
        source_link=str(image_path),
        source="Cloud OCR",
        source_file=image_path.name,
    )
    set_location_tracking(event, "photo_ocr_address" if extracted_address else "unknown", {
        "source_file": image_path.name,
        "ocr_text": raw_text,
...
function openEmbeddedOcrImage(buttonEl) {
    const src = buttonEl.getAttribute("data-image-src");
    const title = buttonEl.getAttribute("data-image-title") || "OCR Sale Image";
    const fileName = buttonEl.getAttribute("data-image-file") || "";
    if (!src) return;
    ...
    const imgHtml = "<!doctype html><html><head><meta charset='utf-8'>" +
        ...
        "header{position:sticky;top:0;background:#1f1f1f;padding:10px 12px;border-bottom:1px solid #444;z-index:2;}" +
        "h1{font-size:15px;margin:0 0 3px 0;} .file{font-size:12px;color:#bbb;}" +
        ".wrap{padding:10px;} img{display:block;max-width:100%;height:auto;margin:0 auto;border-radius:6px;background:white;}" +
        "</style></head><body>" +
        "<header><h1>" + title.replace(/[<>]/g, "") + "</h1><div class='file'>" + fileName.replace(/[<>]/g, "") + "</div></header>" +
        "<div class='wrap'><img src='" + src + "' alt='OCR sale image'></div>" +
        "</body></html>";
    ...
    // Popup blocker fallback: open a same-page overlay instead of using a data: href.
    const old = document.getElementById("ocrImageOverlay");
    if (old) old.remove();
    const overlay = document.createElement("div");
    overlay.id = "ocrImageOverlay";
    overlay.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,.92);z-index:9999999;padding:12px;overflow:auto;";
    overlay.innerHTML = "<button onclick='this.parentElement.remove()' style='position:sticky;top:0;float:right;z-index:2;padding:8px 12px;font-weight:bold;'>Close</button>" +
        "<div style='color:white;font-family:Arial,sans-serif;margin-bottom:8px;'><b>" + title.replace(/[<>]/g, "") + "</b><br><span style='font-size:12px;color:#ccc;'>" + fileName.replace(/[<>]/g, "") + "</span></div>" +
        "<img src='" + src + "' style='display:block;max-width:100%;height:auto;margin:0 auto;background:white;border-radius:6px;'>";
...
def sale_link_html(link: str) -> str:
    if link and link.lower().endswith(IMAGE_EXTENSIONS) and Path(link).exists():
        image_src, mime, _ = image_to_data_url(link)
        if image_src and mime:
            return (
                "<details style='margin-top:6px;'>"
                "<summary style='cursor:pointer; color:blue; font-weight:bold;'>VIEW OCR IMAGE</summary>"
                "<div style='margin-top:6px; max-height:220px; overflow-y:auto; border:1px solid #ccc; padding:4px; background:#fafafa;'>"
                f"<button type='button' "
                f"onclick='openEmbeddedOcrImage(this)' "
                f"data-image-src='{html.escape(image_src, quote=True)}' "
                f"data-image-title='{html.escape('OCR Sale Image', quote=True)}' "
                f"data-image-file='{html.escape(Path(link).name, quote=True)}' "
                f"style='display:block; margin-bottom:5px; color:#1a73e8; background:none; border:0; padding:0; font-weight:bold; cursor:pointer;'>"
                f"Open full image</button>"
                f"<img src='{html.escape(image_src, quote=True)}' alt='OCR sale image' style='max-width:100%; height:auto; border-radius:4px;'>"
                "</div>"
                "</details>"
            )
...
def build_popup_html(event: SaleEvent, sale_id: str) -> str:
    ...
    location_symbol = event.location_symbol or LOCATION_METHOD_SYMBOLS["unknown"]
    location_label = location_method_label(event)
    maps_link_html = (
        f'<span title="{html.escape(location_label)}" style="font-size:16px;">{html.escape(location_symbol)}</span> '
    ...
    location_details_html = pretty_location_details_html(event)
    ...
    return (
        ...
        f"</label>"
        f"</div>"
        f"<details style='margin-top:8px; font-size:11px;'>"
        f"<summary style='cursor:pointer; font-weight:bold;'>"
        f"{html.escape(location_symbol)} Found by: {html.escape(location_label)} <span style='color:#4da3ff;'>| DETAILS</span>"
        f"</summary>"
        f"{location_details_html}"
        f"</details>"
    )
...
def add_marker(mymap: folium.Map, event: SaleEvent, sale_id: str, popup_html: str) -> folium.Marker:
    ...
    if any(k in title_lower for k in MEDIUM_PRIORITY_KEYWORDS):    
        icon = folium.Icon(color="blue", icon="star")
    elif any(k in title_lower for k in HIGH_PRIORITY_NAME_KEYWORDS):
...
def create_map(events: List[SaleEvent]) -> List[Dict[str, Any]]:
    ...
    for idx, event in enumerate(events, start=1):
        ...
        event_record = {
            ...
            "location_method_label": location_method_label(event),
            "location_symbol": event.location_symbol or "❓",
            "location_raw_data": location_debug_payload(event),
        ...
        # For OCR/photo listings, embed the actual image directly in the JSON export.
        # This makes the exported JSON portable: the image can be viewed from data_url,
        # reader_html, or inline_html without needing the original phone file path.
        image_payload = image_attachment_payload(event)
        if image_payload:
            event_record["image_attachment"] = image_payload
